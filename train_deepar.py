...
def parse_data_scaling(dataset, idx,is_train=True, feature=False):# 把所有的销售量进行归一化
    data = []
    ...
    for t in dataset:
        ...
        datai = {FieldName.TARGET: tar[idx,:-12], FieldName.START: t['start']}# 这里放弃最后12个点，即实际训练范围到7.31日下午3点
        if 'id' in t:
        ...
        if feature:
            if is_train:
            # aa=np.concatenate((tar[:idx,:-12],tar[idx+1:,:-12]),axis=0)
            # print(aa.shape)
            # print(np.zeros())
                datai[FieldName.FEAT_DYNAMIC_REAL] = np.concatenate((np.zeros((tar.shape[0]-1,1)),np.concatenate((tar[:idx,:-13],tar[idx+1:,:-13]),axis=0)),axis=1)
            elif not is_train:
                datai[FieldName.FEAT_DYNAMIC_REAL] = np.concatenate(
                    (np.zeros((tar.shape[0] - 1, 1)), np.concatenate((tar[:idx, :-13], tar[idx + 1:, :-13]), axis=0)),
                    axis=1)
        data.append(datai)
    return data
def parse_data_scaling2(dataset, idx,is_train=True, feature=False):# 把除了买一价卖一价之外的进行归一化
    data = []
    scaler = MinMaxScaler(feature_range=(0, 1))
    for t in dataset:
        tar=np.array(t['target'])
        ...
        data.append(datai)
    return data
def parse_data_ar(dataset, idx):
    data = []
    for t in dataset:
        tar = np.array(t['target'])
        datai = {FieldName.TARGET: tar[idx, :-12], FieldName.START: t['start']}
        if 'id' in t:
            datai[FieldName.ITEM_ID] = t['id']
        if 'cat' in t:
            datai[FieldName.FEAT_STATIC_CAT] = t['cat']
    ...
    if args.prepro=="scaling2":
        model_dir = os.path.join('/Users/lvhaoran/AWScode/gluonts/models',
                                 args.algo_name + '_lr' + str(args.learning_rate) + '_epoch' + str(
                                     args.epochs) + 'scaling_all_exp2'+'_usefeat'+str(args.use_feat_dynamic_real))+str(args.tar)
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        train_ds = ListDataset(parse_data_scaling2(train, args.tar,True, args.use_feat_dynamic_real), freq=freq, one_dim_target=True)
        test_ds = ListDataset(parse_data_scaling2(test, args.tar,False, args.use_feat_dynamic_real), freq=freq, one_dim_target=True)
        if args.algo_name == 'DeepAR':
            cardinality = None
        ...
        model_dir = os.path.join('/Users/lvhaoran/AWScode/gluonts/models',
                                 args.algo_name + '_lr' + str(args.learning_rate) + '_epoch' + str(
                                     args.epochs) + 'scaling_all_num' + '_usefeat' + str(args.use_feat_dynamic_real))+str(args.tar)
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        train_ds = ListDataset(parse_data_scaling(train, args.tar, True, args.use_feat_dynamic_real), freq=freq, one_dim_target=True)
        test_ds = ListDataset(parse_data_scaling(test, args.tar, False, args.use_feat_dynamic_real), freq=freq, one_dim_target=True)
         # hybridize=False
        if args.algo_name == 'DeepAR':
            cardinality = None
            trainer = Trainer(ctx="cpu", epochs=args.epochs, learning_rate=args.learning_rate,
        ...
        model_dir = os.path.join('/Users/lvhaoran/AWScode/gluonts/models',
                                 args.algo_name + '_lr' + str(args.learning_rate) + '_epoch' + str(
                                     args.epochs) + 'scaling_all_ar' + '_usefeat' + str(args.use_feat_dynamic_real))+str(args.tar)
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        train_ds = ListDataset(parse_data_ar(train, args.tar), freq=freq, one_dim_target=True)
        test_ds = ListDataset(parse_data_ar(test, args.tar), freq=freq, one_dim_target=True)
        # hybridize=False
        if args.algo_name == 'DeepAR':
            cardinality = None
            trainer = Trainer(ctx="cpu", epochs=args.epochs, learning_rate=args.learning_rate,
